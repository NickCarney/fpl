...
export default function TransferSuggestions({
  ...
  const [parsedAnalysis, setParsedAnalysis] = useState<ParsedTransferAnalysis | null>(null);
  ...
  const parseAnalysis = (analysisText: string): ParsedTransferAnalysis | null => {
    try {
      const lines = analysisText.split('\n').map(line => line.trim()).filter(line => line);
      
      const weaknessStart = lines.findIndex(line => line.includes('WEAKNESS_ANALYSIS'));
      const transferStart = lines.findIndex(line => line.includes('TRANSFER_SUGGESTION'));
      ...
      const player = weaknessSection.find(line => line.startsWith('Player:'))?.replace('Player:', '').trim() || '';
      const position = weaknessSection.find(line => line.startsWith('Position:'))?.replace('Position:', '').trim() || '';
      const issues = weaknessSection.find(line => line.startsWith('Issues:'))?.replace('Issues:', '').trim() || '';

      // Parse transfer suggestion
      const transferSection = lines.slice(transferStart + 1);
      const playerOut = transferSection.find(line => line.startsWith('OUT:'))?.replace('OUT:', '').trim() || '';
      const playerIn = transferSection.find(line => line.startsWith('IN:'))?.replace('IN:', '').trim() || '';
      const reason = transferSection.find(line => line.startsWith('Reason:'))?.replace('Reason:', '').trim() || '';
      const expectedImprovement = transferSection.find(line => line.startsWith('Expected_Improvement:'))?.replace('Expected_Improvement:', '').trim() || '';
      const budgetImpact = transferSection.find(line => line.startsWith('Budget_Impact:'))?.replace('Budget_Impact:', '').trim() || '';
  ...
  return (
    <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200">
      ...
      <div 
        className="flex justify-between items-center p-4 cursor-pointer hover:bg-green-100 rounded-t-lg transition-colors"
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {!isCollapsed && (
            ...
            >
              {loading ? 'Analyzing...' : parsedAnalysis ? 'New Analysis' : 'Analyze Squad'}
            </button>
          )}
        ...
        <div className="px-4 pb-4">
          {loading && (
            <div className="flex items-center justify-center py-8">
              ...
              <p className="text-gray-600">Analyzing transfer opportunities with AI...</p>
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white rounded-lg p-4 border border-gray-200">
                ...
                <div className="bg-red-50 p-3 rounded-lg border border-red-200">
                  <h5 className="font-medium text-red-800 mb-2">Identified Weakness</h5>
                  <p className="text-sm text-red-700 mb-1">
                    <strong>Player:</strong> {parsedAnalysis.weakness.player}
                  </p>
                  <p className="text-sm text-red-700 mb-1">
                    <strong>Position:</strong> {parsedAnalysis.weakness.position}
                  </p>
              ...
              {/* Transfer Suggestion */}
              <div className="bg-white rounded-lg p-4 border border-gray-200">
                <h4 className="font-semibold text-gray-800 mb-3">💡 Recommended Transfer</h4>
                  ...
                  <div className="bg-blue-50 p-3 rounded-lg border border-blue-200 space-y-2">
                    {parsedAnalysis.transfer.reason && (
                      <div>
                        ...
                        <span className="text-sm text-blue-700">{parsedAnalysis.transfer.reason}</span>
                      </div>
                    )}
                    {parsedAnalysis.transfer.expectedImprovement && (
                      <div>
                        <span className="text-sm font-medium text-blue-800">Expected Impact: </span>
                        <span className="text-sm text-blue-700">{parsedAnalysis.transfer.expectedImprovement}</span>
                      </div>
                    )}
                    {parsedAnalysis.transfer.budgetImpact && (
                      <div>
                        <span className="text-sm font-medium text-blue-800">Budget: </span>
                        <span className="text-sm text-blue-700">{parsedAnalysis.transfer.budgetImpact}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {!isFallback && (
                <div className="text-xs text-gray-500 flex items-center gap-1">
                  <span>🤖</span>
                  <span>Powered by AI analysis with live market data, xG stats, and expert insights</span>
                </div>
              )}
            </div>
          )}
          ...
          {analysis && !parsedAnalysis && !loading && (
            <div className="bg-white rounded-lg p-4 border border-gray-200">
              <h4 className="font-semibold text-gray-800 mb-3">Raw Analysis</h4>
          ...
          {!analysis && !loading && (
            <div className="text-center py-8">
              <p className="text-gray-600 mb-4">
                Get AI-powered transfer suggestions based on comprehensive data analysis
              </p>
              <button

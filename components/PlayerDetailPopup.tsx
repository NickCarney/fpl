...
import { Element, Team, ElementType, PlayerGameweekData, PlayerGameweek } from '@/types/fpl';
...
export default function PlayerDetailPopup({
  ...
  const [gameweekData, setGameweekData] = useState<PlayerGameweekData | null>(null);
  ...
  const expectedStats = [
    ...
    { label: 'Expected Goal Involvements (xGI)', value: player.expected_goal_involvements },
    { label: 'Expected Goals Conceded (xGC)', value: player.expected_goals_conceded },
  ];
      ...
      <div className="bg-white rounded-lg p-6 max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <h2 className="text-2xl font-bold">{player.first_name} {player.second_name}</h2>
              {isCaptain && (
                <span className="px-2 py-1 bg-yellow-400 text-sm rounded font-bold">Captain</span>
              )}
              {isViceCaptain && (
                <span className="px-2 py-1 bg-yellow-200 text-sm rounded font-bold">Vice Captain</span>
              )}
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-blue-50 p-4 rounded-lg text-center">
            <div className="text-2xl font-bold text-blue-600">{player.total_points}</div>
            <div className="text-sm text-gray-600">Total Points</div>
          </div>
          <div className="bg-green-50 p-4 rounded-lg text-center">
            <div className="text-2xl font-bold text-green-600">£{(player.now_cost / 10).toFixed(1)}m</div>
            <div className="text-sm text-gray-600">Price</div>
          </div>
          <div className="bg-purple-50 p-4 rounded-lg text-center">
            <div className="text-2xl font-bold text-purple-600">{player.form}</div>
            <div className="text-sm text-gray-600">Form</div>
          </div>
          <div className="bg-orange-50 p-4 rounded-lg text-center">
            <div className="text-2xl font-bold text-orange-600">{player.selected_by_percent}%</div>
              ...
              <table className="w-full text-sm">
                <tbody>
                  {statsRows.map((stat, index) => (
                    <tr key={index} className={stat.highlight ? 'bg-blue-50' : ''}>
                      <td className="py-2 font-medium text-gray-700">{stat.label}</td>
                      <td className="py-2 text-right font-semibold">{stat.value}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {/* Expected Stats */}
        <div className="mt-6">
          <h3 className="text-lg font-semibold mb-3">Expected Stats (Advanced Analytics)</h3>
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              {loadingGameweeks ? (
                <div className="text-center py-4">
                  ...
                  <p className="text-gray-600">Loading gameweek data...</p>
                </div>
              ) : gameweekData ? (
                <div className="overflow-x-auto">
                    ...
                    <tbody>
                      {gameweekData.history.map((gw, index) => (
                        <tr key={index} className="border-b hover:bg-gray-100">
                          ...
                          <td className="py-2">{getOpponentTeamName(gw, teams)}</td>
                          <td className="py-2">{gw.was_home ? 'H' : 'A'}</td>
                          <td className="py-2 text-right font-semibold">{gw.total_points}</td>
                  ...
                  <div className="mt-4 text-xs text-gray-600">
                    <p><strong>CS:</strong> Clean Sheets | <strong>BPS:</strong> Bonus Points System</p>
                  </div>
                </div>
              ) : (
        ...
        <div className="mt-6 bg-gray-50 rounded-lg p-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              <span className="font-medium">Attack (Home):</span> {team.strength_attack_home}
            </div>
            <div>
              <span className="font-medium">Attack (Away):</span> {team.strength_attack_away}
            </div>
            <div>
              <span className="font-medium">Defence (Home):</span> {team.strength_defence_home}
            </div>
            <div>
              <span className="font-medium">Defence (Away):</span> {team.strength_defence_away}
            </div>
          </div>
        </div>

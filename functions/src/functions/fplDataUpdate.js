...
async function hasActiveGames() {
  try {
    ...
    // Find fixtures that have started but not finished
    const activeGames = fixtures.filter(
      (fixture) => fixture.started === true && fixture.finished === false
    );

    if (activeGames.length > 0) {
      console.log(`Found ${activeGames.length} active game(s):`);
      activeGames.forEach((game) => {
        console.log(`  - Fixture ID ${game.id}: ${game.minutes || 0} minutes played`);
      });
      return true;
    }
...
async function upsertElementTypes(pool, elementTypes) {
  console.log(`Upserting ${elementTypes.length} element types...`);
  const transaction = new sql.Transaction(pool);
...
async function performDataUpdate() {
  ...
  console.log('FPL DATA UPDATE - ACTIVE GAME DETECTED');
  console.log('='.repeat(60));
...
app.timer('fplDataUpdate', {
  schedule: '0 * * * * *', // Every minute
  handler: async (myTimer, context) => {
    try {
      context.log('FPL Data Update Timer Triggered');
      context.log(`Current time: ${new Date().toISOString()}`);

      // Check if there are any active games
      const activeGames = await hasActiveGames();

      if (activeGames) {
        context.log('Active game(s) detected - proceeding with data update...');
        await performDataUpdate();
        context.log('Data update completed successfully');
      } else {
        context.log('No active games - skipping update');
      }
    } catch (error) {
      context.log.error('Error in FPL data update:', error);
      throw error;
    }
  }
});
